...
install_pacman_packages() {
  local pacmanfile="$repo_root/arch/Pacmanfile"
  _PACMAN_PACKAGES=()
  _PACMAN_SKIPPED=()
...
enable_hyprmoncfgd() {
  if ! have_command hyprmoncfgd; then
    log "hyprmoncfgd not found after install; skipping monitor profile daemon"
    return 0
  fi
  if ! have_command systemctl; then
    log "systemctl not found; skipping hyprmoncfgd user service"
    return 0
  fi

  log "Enabling hyprmoncfgd user service"
  systemctl --user daemon-reload
  systemctl --user enable --now hyprmoncfgd.service
}

install_aur_packages() {
  local aurfile="$repo_root/arch/Aurfile"
  _AUR_PACKAGES=()
